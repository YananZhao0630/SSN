...
function [Wop, filters] = gong_gabor_factory_fr_3d(size_in, filt_opt, scat_opt, cot_a)
    % Options
	...
	% Create filters
    filt_opt = fill_struct(filt_opt, 'filter_type', 'gabor');
    switch filt_opt.filter_type
        ...
        case 'gabor'
            filt_opt = rmfield(filt_opt, 'filter_type');
            filters = gong_gabor_filter_bank_3d(size_in, filt_opt);
        otherwise
	...
	for m = 1:scat_opt.M+1
        ...
        %% struct the layer filters with variable x
		Wop{m} = @(x)(my_wavelet_layer_fr_3d(x, filters, options_W, cot_a));
    end
end