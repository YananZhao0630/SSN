...
%       低通滤波器sigma_phi (numeric): standard deviation of the low-pass mother
%          wavelet
%       高通（带通）滤波器sigma_psi (numeric): standard deviation of the envelope of the
%          high-pass psi_0
%       xi_psi (numeric): the frequency peak of the band-pass mother
%          wavelet
%       偏心率slant_psi (numeric): excentricity of the elliptic enveloppe of the
...
function filters = Gabor_bank_2d(size_in, options)
    if(nargin<2)
		options = struct;
    end
    ...
    Q = options.Q;
    L = options.L;
	options = fill_struct(options, 'sigma_phi',  3.2);	
    options = fill_struct(options, 'sigma_psi',  3.2);	
%     options = fill_struct(options, 'xi_psi',  1/2*(2^(-1/Q)+1)*pi);
    options = fill_struct(options, 'xi_psi',  0);
...
%     options = fill_struct(options, 'min_margin', options.sigma_phi * 2^(J/Q) );
    options = fill_struct(options, 'min_margin', options.sigma_phi  );
	...
	phi.filter.type = 'fourier_truncated';
...
% 	scale = 2^((J-1) / Q - res);
% 	filter_spatial =  gmorlet_2d_noDC(N, M, sigma_phi*scale, 1, 0, 0);
%% sigma_phi different 3.2 and 6.4
    filter_spatial =  gabor_2d(N, M, sigma_phi, 1, 0, 0);
	...
	for j = 0:J-1
		for theta = 1:numel(angles)			
			angle = angles(theta);
			scale = pi-j*pi/J; %% the key to changing the 
%             scale = 2*pi-2*pi*(j+1)/(J+1);
