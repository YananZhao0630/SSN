...
function filters = stockwell_filter_bank_2d(size_in, options)
    if(nargin<2)
		options = struct;
    end
    ...
    white_list = {'Q', 'L', 'J', 'sigma_phi', 'sigma_psi', ...
		...
		'filter_format', 'chirp_rate', 'basis'};
    ...
    options = fill_struct(options, 'chirp_rate',  1);	
    options = fill_struct(options, 'basis',  0);	
    sigma_phi  = options.sigma_phi;
	sigma_psi  = options.sigma_psi;
	xi_psi     = options.xi_psi;
	slant_psi  = options.slant_psi;
	chirp_rate = options.chirp_rate;
	basis      = options.basis;
    ...
    gamma_f = 2^(-(J-1) / Q - res);
	%scale = 2^((J-1) / Q - res);
	filter_spatial =  gabor_2d(N, M, sigma_phi/gamma_f, 1, 0, 0);
	...
	angles = (0:L-1)  * pi / L; 
    % control the orientation of the wavelet in frequency domain
	p = 1;
	for j = 0:J-1
		for theta = 1:numel(angles)			
			angle = angles(theta);
			gamma_f = 2^(-j/Q - res);
			filter_spatial = stockwell_2d_noDC(N, ...
				M,...
				slant_psi,...
				xi_psi*gamma_f,... 
				angle,...
			...
			%%xi_psi/scale the different scales to construct the morlet wavelets	
