...
if 0
    ...
    im1   = imread('./pic/Sulzberger1_1.bmp');
    im2   = imread('./pic/Sulzberger1_2.bmp');
    im_gt = imread('./pic/Sulzberger1_gt.bmp');
end
...
pixel_vector = reshape(im_di, ylen*xlen, 1); % transfer to one column

fprintf('... ... clustering begin ... ...\n');
im_lab = hclustering(pixel_vector, im_di); %get the three types of classification
...
pos_idx = find(im_lab == 1); % get the position
neg_idx = find(im_lab == 0);
tst_idx = find(im_lab == 0.5);

%% randomize the position index
rand('seed', 2);
...
parfor idx = 1 : size(im1, 2)
    ...
    % change the size with interpolation
...
% === output the results ==============
[FA,MA,OE,CA] = DAcom(im_gt, res_lab);% get the accuracy ratio
fid = fopen('rec.txt', 'a');
