...
function gab = stockwell_2d_noDC(N, M, sigma, slant, xi, theta, scale, chirp_rate, basis, offset)
    ...
    scale1 = chirp_rate*scale + basis; %.85 .05
    %scale1 = scale; %.85 .05
	A = Rth\ [1/((sigma/scale)*scale1)^2, 0 ; 0 slant^2/((sigma/scale)*scale1)^2] * Rth ;
	...
	gab=1/(2*pi*((sigma/scale)*scale1)^2/slant^2)*fftshift(gabc);
